...
// 로그아웃 → /welcome 한 번만 스플래시를 건너뛰기 위한 일회성 신호 (More가 심고 여기서 지운다)
export const SKIP_SPLASH = 'charty:skipsplash'

export default function Welcome() {
  const nav = useNav()
  const session = useSession()
  const setWelcomed = useStore((s) => s.setWelcomed)
  // 로그아웃 직후 재진입이면 스플래시 생략 — 방금 앱을 보고 있던 사람에게 3.8초를 또 보여줄 이유가 없다.
  // 메모리 플래그가 아니라 sessionStorage인 이유: 이 이동이 소프트/하드 내비 어느 쪽이어도 살아남아야 한다.
  // 로그아웃에서만 심으므로 신규 유저의 첫 스플래시에는 영향이 없다 (일회성 — 읽고 지운다)
  const [phase, setPhase] = useState<'splash' | 'start'>(() => {
    try {
      if (!sessionStorage.getItem(SKIP_SPLASH)) return 'splash'
      sessionStorage.removeItem(SKIP_SPLASH)
      return 'start'
    } catch { return 'splash' } // 저장소 접근 불가 환경 — 기존 동작
  })
  ...
  return (
    <div style={{ minHeight: '100dvh', position: 'relative', overflow: 'hidden', background: 'radial-gradient(130% 100% at 50% 0%, #101838 0%, #0A0F26 48%, #06081A 100%)' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.5px', color: '#F5F7FF', marginTop: 22, position: 'relative' }}>Charty</div>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12, position: 'relative' }}>
          <div>
            <button onClick={start} style={{ ...pill, height: 54, border: 'none', background: 'linear-gradient(135deg, #1FB6FF 0%, #3D5CFF 55%, #9B2FF8 100%)', boxShadow: '0 10px 28px rgba(61,92,255,0.35)', color: '#fff', fontSize: 15, fontWeight: 700 }}>
              바로 시작하기
            </button>
          ...
          <button onClick={google} style={{ ...pill, height: 50, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(138,155,255,0.2)', color: '#F2F4FF', fontSize: 14, fontWeight: 600 }}>
